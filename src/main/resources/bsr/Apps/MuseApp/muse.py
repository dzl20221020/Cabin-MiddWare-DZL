...
class Muse():
    ...
    def _init_sample(self):
        ...
        self.timestamps = np.full(4, np.nan)
    ...
    def _init_ppg_sample(self):
        ...
        """
        self.timestamps_ppg = np.full(1, np.nan)
        self.data_ppg = np.zeros((1, 6))
    ...
    def _handle_eeg(self, handle, data):
        ...
        try:
            self.eeg_chann_list.remove(index)        
        except:
            pass
        # last data received
        # if handle == 35:
        if self.eeg_chann_list.__len__()==0:
            if tm != self.last_tm + 1:
                if (tm - self.last_tm) != -65535:  # counter reset
                    print("missing sample %d" % (tm))
    ...
    def _handle_telemetry(self, handle, packet):
        ...
        battery_samp = np.zeros((1, 1))
        battery_ts = np.zeros((1, 1))
        ...
        battery_samp[0,0] = battery
        battery_ts[0,0] = timestamp
        self.callback_telemetry(battery_samp,battery_ts)
